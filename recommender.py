...
class HybridRecommender:
    ...
    def __init__(self):
        self.events_df = None       # датафрейм мероприятий
        self.users_df = None        # датафрейм пользователей
        self.interactions_df = None # датафрейм взаимодействий
        self.interaction_matrix = None  # матрица пользователь-событие
        self.user_factors = None    # латентные факторы пользователей (SVD)
        self.item_factors = None    # латентные факторы мероприятий (SVD)
        self.tfidf_matrix = None    # TF-IDF матрица описаний
        self.tfidf_vectorizer = None
        self.content_features = None # числовые признаки мероприятий
    ...
    def get_cf_scores(self, user_id: int) -> dict:
        ...
        для всех мероприятий для данного пользователя.
        """
        ...
        scores = self.predicted_matrix[u_idx]
    ...
    def recommend(
        ...
        exclude_seen   — исключить уже просмотренные
        ...
        # Получаем оценки обоих компонентов
        cf_scores = self.get_cf_scores(user_id)
        cbf_scores = self.get_cbf_scores(user_id)

        # Мероприятия которые пользователь уже видел
        seen_event_ids = set()
        if exclude_seen and len(self.interactions_df) > 0:
            seen = self.interactions_df[
                self.interactions_df["user_id"] == user_id
            ]["event_id"].tolist()
        ...
        for _, event in self.events_df.iterrows():
            ...
            if exclude_seen and eid in seen_event_ids:
                continue
